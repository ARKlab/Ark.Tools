...
namespace Ark.Tools.Outbox.SqlServer
{
    internal sealed class OutboxContextSqlAsync<Tag> : IOutboxContextAsync
    {
        ...
        public IDbTransaction? Transaction => _sqlContext.Transaction ?? null;
        ...
        public async Task SendAsync(IEnumerable<OutboxMessage> messages, CancellationToken ctk = default)
        {
            foreach (var b in messages.Batch(1000))
            {
                ...
                //await _sqlContext.Create(IsolationLevel.ReadCommitted, ctk);
        ...
        public async Task<int> EnsureTableAreCreated(CancellationToken ctk = default)
        {
            //await _sqlContext.Create(IsolationLevel.ReadCommitted, ctk);
        ...
        public async ValueTask CommitAsync(CancellationToken ctk)
        {
            await _sqlContext.CommitAsync(ctk);
        }
