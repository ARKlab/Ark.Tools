...
- **Low**: diagnostic IDs 002/003 unaccounted; MessagePack 400 lacks ProblemDetails body.
...
| D4 | ICommand HTTP semantics | ~~**`[RebusMessage]`-based alternative**: commands that also carry `[RebusMessage]` (truly async) → `202 Accepted`; synchronously-completed commands → `204 No Content`.~~ **Revised 2026-09-03**: HTTP command endpoints always execute inline and return `204 No Content`; `[RebusMessage]` on the same contract only adds Rebus exposure/routing and never triggers an automatic bus `Send` from the generated HTTP endpoint. |
