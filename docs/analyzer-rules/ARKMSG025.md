# ARKMSG025: MessagePack contract shape is missing

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `Contract '{0}' is used by participant '{1}' with effective protocol MessagePack and must declare MessagePack.MessagePackObjectAttribute`
...
Use the standard `dotnet_diagnostic.ARKMSG025.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMSG025.md`.
