# ARKMF001: Only one attachment is supported

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `HTTP endpoint '{0}' declares more than one IArkAttachment property`
...
Use the standard `dotnet_diagnostic.ARKMF001.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF001.md`.
