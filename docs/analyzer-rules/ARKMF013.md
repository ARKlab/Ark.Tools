# ARKMF013: Invalid contract shape

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `HTTP endpoint '{0}' must be a record with settable properties for body or multipart binding`
...
Use the standard `dotnet_diagnostic.ARKMF013.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF013.md`.
