# ARKMF030: MessagePack is not supported by Azure Functions

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `HTTP endpoint '{0}' enables MessagePack and cannot be selected by an Azure Functions host`
...
Use the standard `dotnet_diagnostic.ARKMF030.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF030.md`.
