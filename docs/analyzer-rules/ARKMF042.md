# ARKMF042: Invalid Storage Queue maximum dequeue count

- **Severity:** Warning
- **Component:** Mediator Framework
- **Diagnostic message:** `host.json extensions.queues.maxDequeueCount must be a positive integer`
...
Use the standard `dotnet_diagnostic.ARKMF042.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF042.md`.
