...
- **Diagnostic message:** `Contract '{0}' has explicit name or alias '{1}', which is not a valid logical name`
...
Use the standard `dotnet_diagnostic.ARKMSG019.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMSG019.md`.
