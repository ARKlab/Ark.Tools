# ARKMF045: Functions messaging subscriber cannot deserialize publisher protocol

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `Participant '{0}' does not support effective protocol '{1}' published by '{2}' for event '{3}'`
...
Use the standard `dotnet_diagnostic.ARKMF045.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF045.md`.
