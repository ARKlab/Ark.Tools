...
- **Diagnostic message:** `Classified member '{0}' has no SqlColumnPolicy on a SqlDataPolicy type; declare its verbatim column name and storage protection`
...
Use the standard `dotnet_diagnostic.ARKPII007.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKPII007.md`.
