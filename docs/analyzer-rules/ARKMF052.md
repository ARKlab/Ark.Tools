# ARKMF052: Use a supported MCP contract

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `MCP contract '{0}' must be a supported request, query, or command`
...
Use the standard `dotnet_diagnostic.ARKMF052.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF052.md`.
