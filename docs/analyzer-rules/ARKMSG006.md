...
- **Diagnostic message:** `Event '{0}' is published by more than one participant in network '{1}'`
...
Use the standard `dotnet_diagnostic.ARKMSG006.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMSG006.md`.
