# ARKMF035: Functions messaging participant has no network

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `Participant '{0}' is not listed by a messaging network`
...
Use the standard `dotnet_diagnostic.ARKMF035.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF035.md`.
