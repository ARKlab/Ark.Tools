...
- **Diagnostic message:** `Rebus contract '{0}' cannot use an IAsyncEnumerable response because Rebus does not enumerate it; the sequence would be discarded and may or may not alter outcomes`
...
Use the standard `dotnet_diagnostic.ARKMF019.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF019.md`.
