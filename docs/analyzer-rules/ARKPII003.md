# ARKPII003: Classified data reaches an exception

- **Severity:** Error
- **Component:** Compliance
- **Diagnostic message:** `Classified member '{0}' ([{1}]) reaches an exception message, data, or error contract; log the key, not the person, or pass the value through a redactor. Analysis is intra-method only`
...
Use the standard `dotnet_diagnostic.ARKPII003.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKPII003.md`.
