# ARKMF053: Use supported MCP input members

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `MCP contract '{0}' has unsupported input member '{1}'; remove or change the member`
...
Use the standard `dotnet_diagnostic.ARKMF053.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF053.md`.
