# ARKMF011: Unsupported handler kind

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `Attributed type '{0}' does not implement a supported handler interface`
...
Use the standard `dotnet_diagnostic.ARKMF011.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF011.md`.
