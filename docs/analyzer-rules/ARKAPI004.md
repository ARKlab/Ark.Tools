# ARKAPI004: Malformed API surface snapshot

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `ArkApiSurface.txt contains an invalid snapshot entry: '{0}'. Messaging entries use multiline blocks; regenerate ArkApiSurface.current.txt and replace the baseline.`
...
Use the standard `dotnet_diagnostic.ARKAPI004.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKAPI004.md`.
