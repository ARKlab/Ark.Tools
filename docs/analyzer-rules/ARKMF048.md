# ARKMF048: Conflicting Azure Functions host version prefixes

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `HTTP host markers for contract assembly '{0}' declare conflicting version prefixes '{1}' and '{2}'`
...
Use the standard `dotnet_diagnostic.ARKMF048.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF048.md`.
