# ARKMF040: Storage Queue host settings are not inspectable

- **Severity:** Info
- **Component:** Mediator Framework
- **Diagnostic message:** `Add host.json to AdditionalFiles so Storage Queue messaging settings can be validated`
...
Use the standard `dotnet_diagnostic.ARKMF040.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF040.md`.
