# ARKMF051: Use a unique MCP tool name

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `MCP tool name '{0}' is declared more than once; rename one tool`
...
Use the standard `dotnet_diagnostic.ARKMF051.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF051.md`.
