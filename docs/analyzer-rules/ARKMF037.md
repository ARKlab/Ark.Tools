# ARKMF037: Functions messaging participant is sender-only

- **Severity:** Info
- **Component:** Mediator Framework
- **Diagnostic message:** `Participant '{0}' consumes no contracts, so no receive trigger is generated`
...
Use the standard `dotnet_diagnostic.ARKMF037.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF037.md`.
