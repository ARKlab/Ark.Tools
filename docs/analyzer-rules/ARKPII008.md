...
- **Diagnostic message:** `Compliance review on '{0}' has no reason or has an invalid or expired Expires date; an unreviewed exception can expose personal data`
...
Use the standard `dotnet_diagnostic.ARKPII008.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKPII008.md`.
