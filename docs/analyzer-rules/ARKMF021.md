# ARKMF021: Contract has multiple Solid kinds

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `Contract '{0}' can implement only one of IQuery, IRequest, or ICommand`
...
Use the standard `dotnet_diagnostic.ARKMF021.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF021.md`.
