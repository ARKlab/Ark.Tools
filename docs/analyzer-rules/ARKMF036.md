# ARKMF036: Functions messaging participant has multiple networks

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `Participant '{0}' is listed by more than one messaging network`
...
Use the standard `dotnet_diagnostic.ARKMF036.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF036.md`.
