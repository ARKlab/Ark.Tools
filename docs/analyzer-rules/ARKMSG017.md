...
- **Diagnostic message:** `Retry policy for participant '{0}' must have MaximumDeliveryCount >= {1}`
...
Use the standard `dotnet_diagnostic.ARKMSG017.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMSG017.md`.
