# ARKMF039: Functions messaging subscription has no publisher

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `Subscribed event '{0}' does not have exactly one publisher in network '{1}'`
...
Use the standard `dotnet_diagnostic.ARKMF039.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF039.md`.
