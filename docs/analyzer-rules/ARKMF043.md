# ARKMF043: Invalid Storage Queue visibility timeout

- **Severity:** Warning
- **Component:** Mediator Framework
- **Diagnostic message:** `host.json extensions.queues.visibilityTimeout must be a positive TimeSpan`
...
Use the standard `dotnet_diagnostic.ARKMF043.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF043.md`.
