# ARKMF020: Version prefix is missing the version token

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `The version prefix must contain the '{version}' token`
...
Use the standard `dotnet_diagnostic.ARKMF020.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF020.md`.
