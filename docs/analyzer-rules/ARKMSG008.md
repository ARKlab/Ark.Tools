...
- **Diagnostic message:** `Participant '{0}' subscribes to event '{1}', which is not published in network '{2}'`
...
Use the standard `dotnet_diagnostic.ARKMSG008.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMSG008.md`.
