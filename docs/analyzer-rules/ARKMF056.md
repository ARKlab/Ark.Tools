# ARKMF056: Declare the MCP context as partial

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `MCP context must be declared partial`
...
Use the standard `dotnet_diagnostic.ARKMF056.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF056.md`.
