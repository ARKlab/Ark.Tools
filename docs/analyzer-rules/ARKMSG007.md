...
- **Diagnostic message:** `Contract '{0}' is not owned by a participant in network '{1}'`
...
Use the standard `dotnet_diagnostic.ARKMSG007.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMSG007.md`.
