# ARKMF046: Messaging native entity name collision

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `Logical messaging names '{0}' and '{1}' map to the same {2} entity name '{3}'`
...
Use the standard `dotnet_diagnostic.ARKMF046.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMF046.md`.
