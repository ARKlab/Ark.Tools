# ARKMSG026: Google.Protobuf contract shape is missing

- **Severity:** Error
- **Component:** Mediator Framework
- **Diagnostic message:** `Contract '{0}' is used by participant '{1}' with effective protocol Protobuf and must implement Google.Protobuf.IMessage<T> and expose the generated parser shape`
...
Use the standard `dotnet_diagnostic.ARKMSG026.severity` EditorConfig setting only for an intentional exception. Prefer a documented, reviewed suppression over disabling the rule globally.

## Source

This rule is implemented in Ark.Tools and its descriptor links here: `https://github.com/ARKlab/Ark.Tools/blob/master/docs/analyzer-rules/ARKMSG026.md`.
