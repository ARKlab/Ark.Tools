...
**Depends on**: AZM-01 through AZM-15, including AZM-07A
...
- **API baseline**: update repository API-surface snapshots for every public
  attribute, enum, option, `IBus`, `IFailed<T>`, pipeline, DataBus, and context
  member; verify the AZM-03 message/event name, owner, and alias entries.
...
- **Release gate**: search for stale old task names, direct-subscription claims,
  Rebus interoperability claims, technology-typed network claims, host-local
  pipeline settings, and durable AMF
  outbox claims.

## Implementation steps

1. Update the Mediator Framework guide with contract declarations, host
...
6. Document the incoming/outgoing pipeline, opt-in user/OTel propagation,
   additional headers, compression, DataBus claim-check, provider-specific
   minimum lifetime, and the Azure Blob IaC lifecycle prerequisite.
7. Document that durable Mediator Framework outbox support, request/reply,
   and delayed publish are out of scope; document each transport's capability
   set (Storage Queue has no `PubSub`; its DLQ is the framework-managed
   poison queue); distinguish the
   non-durable passthrough outbox.
...
10. Document the Rebus durable-outbox versus Mediator Framework passthrough
    commit-then-send durability tradeoff. State that the registered bus backend
    selects the outbox, and that the existing WebInterface and RebusProcessor
    retain their real Rebus outbox registrations.
11. Document logical contract names, `FormerNames`, and the explicit topology
    migration required when an event topic name changes.
12. Document the portable queue-name convention and analyzer diagnostics.
...
- Composition examples preserve the WebInterface Rebus outbox with its
  ...
  passthrough appears only with the native Mediator Framework bus.
...
- [ ] Rebus compatibility boundaries and the durable-outbox/passthrough
  distinction are explicit.
