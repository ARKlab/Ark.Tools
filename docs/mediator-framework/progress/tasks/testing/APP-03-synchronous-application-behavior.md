...
| Failure/dead-letter behavior | `FailingRebusRequest` where application-owned | Typed failure, second-level `IFailed<T>` handling, and eventual error outcome when dispatched through Rebus |
