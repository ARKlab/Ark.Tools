...
1. Open an issue document (e.g., `01-file-scoped-namespaces.md`)
2. Copy the content
3. Create a new GitHub issue with the document's title
4. Paste content as issue body
5. Add labels: `modernization`, `dotnet-10`, `performance`, priority level

---

## Top 10 Quick Wins

Quick wins ordered by impact/effort ratio for fast value delivery.

### 1. File-Scoped Namespaces ⚡ **EASY** (5 min)
- Reduces indentation, improves readability
- Use IDE "Convert to file-scoped namespace" on all files
- Update .editorconfig: `csharp_style_namespace_declarations = file_scoped:warning`

### 2. Dictionary.TryAdd() 🎯 **EASY** (10 min)
- Cleaner code, slight performance gain
- Replace `if (!dict.ContainsKey(key)) dict.Add(key, value)` with `dict.TryAdd(key, value)`

### 3. LINQ Count Optimization 📊 **EASY** (15 min)
- Fewer iterations, better performance
- Replace `.Where(predicate).Count()` with `.Count(predicate)`
- Replace `.Where(predicate).Any()` with `.Any(predicate)`

### 4. Global Usings 📝 **EASY** (30 min)
- Reduces boilerplate
- Add to Directory.Build.props or .csproj files

### 5. ReadyToRun Compilation 🚀 **EASY** (5 min)
- 30-50% faster startup
- Add `<PublishReadyToRun>true</PublishReadyToRun>` to sample applications

### 6. Collection Expressions 📚 **EASY** (5 min)
- Modern, consistent syntax
- Replace `new[] { 1, 2, 3 }` with `[1, 2, 3]`

### 7. SearchValues for Validation ⚡ **MEDIUM** (1-2 hours)
- 10-100x faster character validation
- Create static SearchValues for string validation scenarios

### 8. Span<T> for String Parsing 🎯 **MEDIUM** (2-4 hours)
- Zero-allocation string operations
- Focus on EnumerableExtensions.cs hot paths

### 9. FrozenDictionary for Static Lookups 🧊 **MEDIUM** (1 hour)
- 20-30% faster lookups
- Replace static `Dictionary<K,V>` with `FrozenDictionary<K,V>`

### 10. Assembly Trimming 📦 **MEDIUM** (2-3 hours)
- 30-40% smaller deployments
- Enable in phases (see Issue #08)
...
### ✅ Strengths
- Latest stable dependencies (Polly 8.6.5, Rebus 8.9.0, Flurl 4.0.2, SimpleInjector 5.5.0)
- Strong analyzer configuration (NetAnalyzers, Meziantou.Analyzer, BannedApiAnalyzers)
- Good async/await practices (343 ConfigureAwait usages)
- Proper JsonSerializerOptions caching
- **Zero `.Substring()` usages** - already following best practices

### ⚠️ Opportunities for Improvement
- Minimal Span<T>/Memory<T> adoption (only 2 files out of 61 projects)
- Block-scoped namespaces instead of file-scoped
- No global usings
- SearchValues<T> optimization opportunities
- LINQ anti-patterns (Where().Count(), ContainsKey+Add)
...
- **Action Needed:** IDE0160/IDE0161 (file-scoped namespaces), MA0029 (LINQ - upgrade to warning)
...
- ✓ Issue #01: File-scoped namespaces
- ✓ Issue #02: Global usings
- ✓ Issue #03: LINQ optimizations
- ✓ Collection expressions (3 locations)

**Expected Impact:** 5-10% build time improvement, better readability

### Week 2: Performance Optimizations
- ○ Issue #05: SearchValues implementation
...
- ○ Issue #07: ReadyToRun + SingleFile
- ○ Issue #04: Span<T> for string parsing (start)

**Expected Impact:** 30-50% faster startup, reduced allocations

### Weeks 4+: Long-term Initiatives
- ○ Issue #08: Trimming Phase 1
- ○ Issue #04: Span<T> continued optimization

**Expected Impact:** 30-40% smaller deployments
...
The sections below provide in-depth technical guidance for each modernization opportunity.
