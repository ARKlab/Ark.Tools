...
namespace Ark.Tools.AspNetCore.XlsxOutputFormatter.Serialisation
{
    ...
    public class DefaultXlsxSerialiser : IXlsxSerialiser
    {
        ...
        {
            var rowValue = ReflectionHelper.GetFieldOrPropertyValue(rowObject, name);

            if (rowValue is DateTimeOffset)
                return ((DateTimeOffset)rowValue).ToString("O");
        ...
        private static object ConvertFromDateTimeOffset(DateTimeOffset dateTime)
        {
