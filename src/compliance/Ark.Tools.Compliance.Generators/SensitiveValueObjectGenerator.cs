...
public sealed class SensitiveValueObjectGenerator : IIncrementalGenerator
{
    ...
    private static string _emit(INamedTypeSymbol type, Settings settings)
    {
        ...
        builder.AppendLine("    /// <param name=\"category\">The GDPR-style category of the processing this reveal serves.</param>");
        builder.AppendLine("    /// <returns>The cleartext value.</returns>");
        builder.AppendLine("    public string Reveal(global::Ark.Tools.Compliance.CompliancePurpose purpose, global::Ark.Tools.Compliance.CompliancePurposeCategory category)");
        builder.AppendLine("    {");
        ...
        builder.AppendLine("        if (category == global::Ark.Tools.Compliance.CompliancePurposeCategory.Unspecified || !global::System.Enum.IsDefined(category))");
        builder.AppendLine("            throw new global::System.ArgumentException(\"A defined compliance purpose category is required.\", nameof(category));");
