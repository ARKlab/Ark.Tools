...
namespace ODataSample.Controllers
{
	...
	public class CountriesController : ODataController
	{
		...
		public IActionResult Put_Countries(int key, [FromBody] Country update)
		{
			...
			//void upd<T>(EntityEntry<T> current, T put, DbContext ctx)
			//	where T : class
			//{
			//	current.CurrentValues.SetValues(put);

			//	foreach (var n in current.Navigations)
			//	{
			//		if (n.CurrentValue is ICollection)
			//	}					

			//}

			//upd(_db.Entry(country), update, _db);
